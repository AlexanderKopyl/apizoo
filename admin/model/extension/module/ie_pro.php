<?php
    class ModelExtensionModuleIePro extends Model {
        ...
        public function format_number_thousand_separator($number) {
            ...
            else if (strpos($number, ',') !== false && !strpos($number, '.') && strlen(explode(",", $number)[1]) > 2)
                $number = str_replace(',', '', $number);
            //For example 1,25 is really 1.25
            else if (strpos($number, ',') !== false && !strpos($number, '.') && strlen(explode(",", $number)[1]) <= 2)
                $number = str_replace(',', '.', $number);
        ...
        public function get_column_index_by_field($file_columns = array(), $table, $field, $identificator = '') {
            $index_column = 0;
            foreach ($this->custom_columns as $key => $col_info) {
                ...
                if($meet_identificator && !empty($col_info['table']) && $col_info['table'] == $table && !empty($col_info['field']) && $col_info['field'] == $field)
    ...
    class ConditionalExpressionEvaluator {
        ...
        private function evaluate_condition_value( $column_name, $element, $cond, $custom_name) {
            ...
            if (!empty($column_name) && isset( $element[$column_name])) {
                ...
                if (!empty( $custom_name) && isset($element[$custom_name]) && (!empty($element[$custom_name]) || $element[$custom_name] === 0)) {
